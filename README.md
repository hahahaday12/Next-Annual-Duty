...
3. header에 유저 정보를 가져오고 프로필 이미지 가져오는것 swr 을 이용해서 자체적으로 hook을 만듬. hook 안에서 userinfo의 api를 get한후에 서버에서 오는 이미지를 출력해야 하는 상황. <br>
서버에서 오는 데이터의 형태는 아래와 같이 json 형태 <br>
